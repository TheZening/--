...
\begin{mdefinition}{量子力学中的格林函数}
量子力学中的格林函数定义为满足如下方程的函数 $G(x, x'; z)$:
\begin{equation}
(z - \hat{H}_x) G(x, x'; z) = \delta(x - x')
\end{equation}
其中格林函数是Hamiltonian 的 Resolvent 在坐标表象下的矩阵元:
\begin{equation}
...
我们必须小心区分这两个概念.
从这角度上讲, 格林函数就是 Hamiltonian 的 Resolvent 在坐标表象下的核:
\begin{equation}
G(x, x'; z) = \mel{x}{(z\mathbb{1} - \hat{H})^{-1}}{x'}
\end{equation}
我们也可以把他写成本征态展开的形式:
...
另外, 我们可以直接通过调节 $z$ 的虚部来控制格林函数的解析性质.
如果我们选择把 $z$ 写成 $E + \mathrm{i}\,\epsilon$, 其中 $\epsilon$ 是一个非常小的正实数, 那么我们就自然的引入了自然因果关系, 这就是之前我们提到的 retarded Green function (推迟格林函数).
\begin{equation}
\tilde{G}^R(E) = \frac{1}{E - \hat{H} + \mathrm{i}\,\epsilon}
\end{equation}
同样的, 如果我们选择把 $z$ 写成 $E - \mathrm{i}\,\epsilon$, 其中 $\epsilon$ 是一个非常小的正实数, 那么我们就自然的引入了反因果关系, 这叫做 \newterm{advanced Green function (前进格林函数)}, 对应于:
\begin{equation}
\tilde{G}^A(E) = \frac{1}{E - \hat{H} - \mathrm{i}\,\epsilon}
\end{equation}


现在我们就以上面的记号为基础, 来解释为什么 $+\mathrm{i}\,\epsilon$ 对应于推迟格林函数.
\comment{实际上我们第一节就计算过这个问题了, 我们这里再重复一遍, 以加深印象.}
我们考虑能量域延迟格林函数:
\begin{equation}
\tilde{G}^R(E) = \frac{1}{E - \hat{H} + \mathrm{i}\,\epsilon}
\end{equation}
...
我们有:
\begin{equation}
\left( \pdv[2]{}{x} + k^2 \right) \tilde{G}(x, x'; z) = \frac{2m}{\hbar^2} \delta(x-x')
\end{equation}
